...
#[test]
fn build_job_views_detect_waiting_upstream_input_after_planning_stall() {
    ...
    assert_eq!(jobs.len(), 1);
    assert_eq!(jobs[0].status, anima_web::jobs::JobStatus::WaitingUpstreamInput);
    assert_eq!(jobs[0].status_label, "waiting_upstream_input");
    assert!(jobs[0].current_step.contains("等待上游交互输入"));
}
...
fn build_job_views_waiting_upstream_input_does_not_override_completed_message() {
    ...
    assert!(jobs[0].current_step.contains("执行完成"));
...
fn status_api_exposes_runtime_summary() {
    ...
    let response = tokio_rt
        .block_on(async {
            app.oneshot(Request::builder().uri("/api/status").body(Body::empty()).unwrap())
                .await
        })
        .unwrap();
