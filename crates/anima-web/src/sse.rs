        ...
        loop {
            ...
            match rx.recv_timeout(Duration::from_millis(100)) {
                Ok(msg) => match &msg.payload {
                    ...
                    {
                        let phase = if event.ends_with("started") {
                            "started"
                        } else {
                            "stopped"
                        };
...
fn map_stream_delta(message_id: &str, payload: &Value) -> Option<SseEvent> {
    ...
    let (kind, delta) = match delta_kind {
        ...
        "thinking_delta" => (
            "thinking",
            payload.get("thinking_delta").and_then(|v| v.as_str())?,
        ),
        "input_json_delta" => (
            "tool_input",
            payload.get("partial_json").and_then(|v| v.as_str())?,
        ),
        _ => return None,
    };
...
mod tests {
    ...
    fn map_text_delta() {
        ...
        assert!(
            matches!(sse, SseEvent::StreamDelta { kind, delta, index, .. } if kind == "text" && delta == "hello" && index == 1)
        );
    }
    ...
    fn map_thinking_delta() {
        ...
        assert!(
            matches!(sse, SseEvent::StreamDelta { kind, delta, .. } if kind == "thinking" && delta == "step 1")
        );
    }
